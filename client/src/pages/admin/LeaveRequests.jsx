  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        {tabs.map(t => (
          <button key={t.value} onClick={() => setFilter(t.value)}
            className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${
              filter === t.value ? 'bg-white/5 shadow-sm text-white' : 'text-white/40 hover:text-white/60'
            }`}>
            {t.label}
      ...
      <div className="space-y-3">
        {filtered.length === 0 ? (
          <div className="card text-center text-white/30 py-12">No leave requests found</div>
        ) : filtered.map(l => (
            ...
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-white/30 text-sm font-mono">{l.emp_id}</span>
                ...
                {l.reason && <p className="text-sm text-white/40">"{l.reason}"</p>}
                {l.admin_comment && <p className="text-sm text-white/30 mt-1 italic">Admin: {l.admin_comment}</p>}
                <p className="text-xs text-white/30 mt-2">Applied {new Date(l.applied_on).toLocaleDateString('en-IN')}</p>

                {warnings[l.id]?.map((w, i) => (
                  <div key={i} className="mt-2 flex items-start gap-2 text-amber-400 bg-amber-50 rounded-lg p-2 text-sm">
